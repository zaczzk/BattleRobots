...
{
    [CreateAssetMenu(menuName = "BattleRobots/Arena/ZoneControlCaptureCechCohomology", order = 478)]
    public sealed class ZoneControlCaptureCechCohomologySO : ScriptableObject
    {
        [Header("Config")]
        [SerializeField, Min(1)] private int _simplicesNeeded    = 7;
        [SerializeField, Min(1)] private int _collapsePerBot     = 2;
        [SerializeField, Min(0)] private int _bonusPerResolution = 3910;
        ...
        public int RecordPlayerCapture()
        {
            ...
            if (_simplices >= _simplicesNeeded)
            {
                ...
                _simplices          = 0;
                _onCechCohomologyResolved?.Raise();
                return bonus;
            }
            return 0;
        }
